...
class UserController{
    static async register(req:Request,res:Response){
        ...
        await sendMail({
            to : email, 
            subject : "Registration successfull on Pasal", 
            text : "Welcome to Pasal, Thank you for registering"
        })
    ...
    static async login(req:Request,res:Response){
        ...
        const {email, password} = req.body // password - bimal --> hash() --> $234234324fjlsdf
        if(!email || !password){
            res.status(400).json({
    ...
    static async handleForgotPassword(req:Request,res:Response){
        ...
        await sendMail({
            to : email, 
            subject : "Bimal Pasal Password Change Request", 
